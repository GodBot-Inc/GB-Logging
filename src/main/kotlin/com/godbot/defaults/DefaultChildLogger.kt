...
class DefaultChildLogger(
    ...
    fun info(msg: String) {
        if (!collectiveLogging && loggingLevel >= lowestLoggingLevel)
            ...
            parentLogger.saveLog(DefaultLog(getId(), "info", loggingLevel, msg, 1))
    }
    fun warning(msg: String) {
        if (!collectiveLogging && loggingLevel >= lowestLoggingLevel)
            super.warning(msg, loggingLevel)
        else if (collectiveLogging && loggingLevel >= lowestLoggingLevel)
            parentLogger.saveLog(DefaultLog(getId(), "warning", loggingLevel, msg, 1))
    }
    fun error(msg: String) {
        if (!collectiveLogging && loggingLevel >= lowestLoggingLevel)
            super.error(msg, loggingLevel)
        else if (collectiveLogging && loggingLevel >= lowestLoggingLevel)
            parentLogger.saveLog(DefaultLog(getId(), "error", loggingLevel, msg, 1))
    }
    fun fatal(msg: String) {
        if (!collectiveLogging && loggingLevel >= lowestLoggingLevel)
            super.fatal(msg, loggingLevel)
        else if (collectiveLogging && loggingLevel >= lowestLoggingLevel)
            parentLogger.saveLog(DefaultLog(getId(), "fatal", loggingLevel, msg, 1))
    }
}
